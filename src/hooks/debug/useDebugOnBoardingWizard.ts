...
interface UseDebugOnBoardingWizardProps {
    isDebug: boolean 
}
...
const useDebugOnBoardingWizard = ({ isDebug }: UseDebugOnBoardingWizardProps) => {
  const adminConfig = useContext(adminConsoleContext)
  const navigate = useNavigate()
  const { handleUpdateStep, handleAddStep } = useResetOnBoardingWizardTest()
...
export default useDebugOnBoardingWizard