...
import {
  ApplicationValidator, ApplicationValidatorFields 
} from '../../../core/validation/validators/ApplicationValidator'
import {
...
const useApplicationFormValidation = () => {
  ...
  const validateField = (field: ApplicationValidatorFields, data: CreateEdfiApplicationRequest | UpdateEdfiApplicationRequest) => {
    handleSingleError({ 
      field, 
      error: ApplicationValidator.validateField({
        data,
        field 
      }) 
    })
  ...
  const validApplicationData = (data: CreateEdfiApplicationRequest | UpdateEdfiApplicationRequest) => {
    const nameError = ApplicationValidator.validateField({
      data,
      field: 'applicationName' 
    })

    const claimsetError = ApplicationValidator.validateField({
      data,
      field: 'claimset' 
    })

    const vendorError = ApplicationValidator.validateField({
      data,
      field: 'vendor' 
    })
    ...
    if (nameError) {
      formDataErrors['applicationName'] = nameError
    }

    if (claimsetError) {
      formDataErrors['claimset'] = claimsetError
    }

    if (vendorError) {
      formDataErrors['vendor'] = vendorError
    }
