...
import { PartnerValidator, PartnerValidatorFields } from "../../../core/validation/validators/PartnerValidator"
...
const usePartnerFormValidation = () => {
    ...
    const validateInputChange = (field: PartnerValidatorFields, data: CreateEdfiVendorRequest) => {
        handleSingleError({ 
            field, 
            error: PartnerValidator.validateField({ data, field }) 
        })
    }

    const validPartnerData = (data: CreateEdfiVendorRequest) => {
        const nameError = PartnerValidator.validateField({ data, field: 'partnerName' })
        const namespacePrefixes = PartnerValidator.validateField({ data, field: 'namespacePrefixes' })
        
        const formDataErrors: FormDataErrors = {}

        if (nameError) formDataErrors["partnerName"] = nameError
        if (namespacePrefixes) formDataErrors["namespacePrefixes"] = namespacePrefixes
